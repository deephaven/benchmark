...
public class RollingCountTimeTest {
    ...
    void rollingCountTime0Group3Ops() {
        ...
        runner.test("RollingCountTime- 3 Ops No Groups", q, "num1", "timestamp");
    }

    @Test
    void rollingCountTime1Group3Ops() {
        var q = "timed.update_by(ops=[contains_row, before_row, after_row], by=['key1'])";
        runner.test("RollingCountTime- 3 Ops 1 Group 100 Unique Vals", q, "key1", "num1", "timestamp");
    }

    @Test
    void rollingCountTime2Groups3Ops() {
        var q = "timed.update_by(ops=[contains_row,before_row,after_row],by=['key1','key2'])";
        runner.test("RollingCountTime- 3 Ops 2 Groups 10K Unique Combos", q, "key1", "key2", "num1", "timestamp");
    }

    @Test
    void rollingCountTime3Groups3Ops() {
        var q = "timed.update_by(ops=[contains_row,before_row,after_row],by=['key1','key2','key3'])";
        runner.test("RollingCountTime- 3 Ops 3 Groups 100K Unique Combos", q, "key1", "key2", "key3", "num1",
                "timestamp");
    }
